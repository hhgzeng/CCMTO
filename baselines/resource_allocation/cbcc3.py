"""
CBCC3: Contribution-Based Cooperative Co-evolution 3 (Omidvar et al. 2016).

Maintains a balance between exploration and exploitation using an epsilon-greedy strategy.
"""
...
class CBCC3(CCMTO):
    """
    CCMTO variant with CBCC3 resource allocation.
    ...
    def optimize(self) -> Dict[str, Union[np.ndarray, float, int, List]]:
        ...
        while self.fe_counter[0] < self.max_fes:
            ...
            # Phase 2: Epsilon-Greedy selection (CBCC3)
            ...
            for _ in range(phase2_steps):
                ...
                if np.random.rand() < self.epsilon_greedy:
                    # Exploration: random MTOP
                    chosen_idx = int(np.random.randint(k_mtops))
                else:
                    # Exploitation: best MTOP
                    chosen_idx = int(np.argmax(allocator.contributions))
