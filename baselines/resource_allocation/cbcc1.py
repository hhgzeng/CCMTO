"""
CBCC1: Contribution-Based Cooperative Co-evolution 1 (Omidvar et al. 2011).

Allocates computational resources to MTOPs using roulette-wheel selection
proportional to their normalized fitness contributions.
"""
...
class CBCC1(CCMTO):
    """
    CCMTO variant with CBCC1 resource allocation.
    In Phase 2, MTOPs are selected with probability proportional to fitness contribution.
    """
    ...
    def optimize(self) -> Dict[str, Union[np.ndarray, float, int, List]]:
        ...
        while self.fe_counter[0] < self.max_fes:
            ...
            # Phase 2: Proportional selection (CBCC1 roulette-wheel)
            phase2_steps = max(1, k_mtops * 2)
            for _ in range(phase2_steps):
                if self.fe_counter[0] >= self.max_fes:
                    break

                contribs = np.maximum(allocator.contributions, 0.0)
                sum_c = np.sum(contribs)
                if sum_c > 1e-12:
                    probs = contribs / sum_c
                    probs = 0.8 * probs + 0.2 * (np.ones(k_mtops) / k_mtops)
                else:
                    probs = np.ones(k_mtops) / k_mtops

                selected_idx = int(np.random.choice(k_mtops, p=probs))
                self.best_x, self.best_f = allocator.optimize_mtop(
                    mtop_idx=selected_idx,
                    best_x=self.best_x,
                    best_f=self.best_f,
                    max_fes=self.max_fes,
                    fe_counter=self.fe_counter,
