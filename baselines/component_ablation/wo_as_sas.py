"""
wo-AS-SaS: CCMTO-MTES-DAKG without AS-SaS (uses DT-DoS only).

Uses proposed Dynamic Distance Threshold Domain KGxS with Gradient Correction (DT-DoS)
along with standard Shape KGxS (unweighted top mu elite center).
...
from .wo_da import standard_shape_kgxs


class MTES_wo_AS_SaS_Optimizer:
    """MTES optimizer without AS-SaS (DT-DoS + standard shape KGxS)."""
...
class WO_AS_SaS(CCMTO):
    """CCMTO with wo-AS-SaS (without AS-SaS, with DT-DoS only)."""
