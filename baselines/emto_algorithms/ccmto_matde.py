"""
CCMTO-MaTDE: Many-Task Differential Evolution (Chen et al. 2020) inside the CCMTO framework.

Replaces MTES-DAKG with MaTDE as the EMTO optimizer for solving constructed MTOPs.
Uses inter-task transfer archive and adaptive crossover.
"""
...
class MaTDE_Optimizer:
    """Many-Task Differential Evolution optimizer for a single MTOP."""
    ...
    def __init__(
        ...
        # Task dimensions and populations
    ...
    def optimize(
        ...
        # Initialize / sync population with current collaborator
        ...
        for gen in range(1, self.max_gen + 1):
            ...
            for k in list(active_tasks):
                ...
                for i in range(self.pop_size):
                    ...
                    # Multitask Transfer vs Intra-task DE
                    if (np.random.rand() < self.p_transfer) and (len(active_tasks) > 1):
                        # Inter-task knowledge transfer
                        other_k = int(np.random.choice([s for s in active_tasks if s != k]))
                        donor_task_pop = self.populations[other_k]
                        r_idx = np.random.randint(self.pop_size)
                        donor_vec = donor_task_pop[r_idx]

                        # Adjust dimension via truncation or zero-padding / tiling
                        donor_proj = np.zeros(d)
                        copy_len = min(d, len(donor_vec))
                        donor_proj[:copy_len] = donor_vec[:copy_len]
                        if d > copy_len:
                            donor_proj[copy_len:] = pop[i, copy_len:]

                        # DE crossover with donor
...
class CCMTO_MaTDE(CCMTO):
    """CCMTO with MaTDE (Many-Task DE) as EMTO solver."""
        ...
        while self.fe_counter[0] < self.max_fes:
            ...
            for i in range(k_mtops):
                if self.fe_counter[0] >= self.max_fes:
                    break
                f_last = self.best_f
                best_sols, _, stagnant_sets[i] = matde_opts[i].optimize(
            ...
            while (np.max(contributions) - np.min(contributions) > self.epsilon) and self.fe_counter[0] < self.max_fes:
                ...
                f_last = self.best_f
                best_sols, _, stagnant_sets[best_idx] = matde_opts[best_idx].optimize(
                    collaborator=self.best_x,
